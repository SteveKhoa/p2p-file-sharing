...
class Peer:
    ...
    def connect_with_peer(self, other_peer_host, other_peer_port):
        ...
        '''
        try:
            connection_edge = self._socket.connect((other_peer_host,other_peer_port))
            self._connections.append(connection_edge)
    ...
    def listening_to_connect(self):
        ...
        while True:
            connection_edge, other_peer_address = self._socket.accept()
            self._connections.append(connection_edge)
    ...
    def receive_msg(self):
        ...
        '''
        (msg ,  other_peer_address) = self._socket.recvfrom(1024)
        if msg == "stop":
            self._socket.close();
            return
        # (other_peer_host, other_peer_port) = socket.getnameinfo(other_peer_address, True)
    ...
    def start(self):
        ...
        '''
        self._listen_thread = threading.Thread(target = self.listening_to_connect)
